...
## Wave 11 (2026-09-21, from the 20260921-full leg diagnoses)

| Patch (commit title) | What the runs showed | Upstream PR? |
|---|---|---|
| `fix(macos): classify the window-sharing indicator on launch_app's rows too` (65034fa59) | Capture-lease artifact classified once, by the driver. The 66×20 `"Window"` row reached the model through `launch_app`, which never classified anything (`kind = None` on every row): `20260921-full/native-act-finder-sort/omp-1` L27 lists it **first**, ahead of the six real Finder rows (`window_id: 30173`, layer 0, `is_on_screen: false`, `z_index: 194`), and OMP printed that text verbatim — the one live leak in the whole run corpus. Two unrelated patterns each hid the same artifact on their own path: OMP's title+geometry test and the driver's byte-exact XPC-path enclosure test; they are not redundant, since a stale indicator (lease ended, window kept) no longer encloses a provider view and only the geometry one still matched it. The driver does not create that window — macOS draws it inside the captured application's process — so the honest fix is one classification, owned by the driver: `launch_app` now computes the overlay set from the pre-filter enumeration the enclosure test needs (it holds the whole roster, like `list_windows`), publishes `kind: "system_overlay"` on its rows and keeps them out of the summary it offers to act on. The indicator's AX detail (`AXDialog` titled `"Window"`, single child `AXButton "WindowSharingSessionButton"`) moved from OMP's deleted pattern onto `SYSTEM_OVERLAY_KIND`, the one kind constant with no doc comment. Not closed here: an indicator in the stale state carries no classification from either side until the lease records the id it caused (note 01 S3, deferred — it needs a live lease diff nobody has run; S4's tighter size band must not ship without it). | yes for the `launch_app` classification — a driver-side truth upstream also lacks; OMP reading `kind` instead of re-deriving it is fork-local |
| `test(macos): fixture scenarios for the caret, descendant text and the desktop row` (a8b7c5ff3) | Fixture scenarios for the three wave-9 features; bench strings out of the unit tests. Caret, descendant text and the desktop row shipped with unit tests only (`tree.rs:1649` transcribed a bench note verbatim, `tree.rs:1611` / `get_window_state.rs:1926` seed Notes' private AX identifier), while wave 10 added seven scenarios beside them. Three env-gated AppKit scenarios now exercise each one against a generic app — an editable `NSTextView` for the caret (UTF-16 offset proven by the app's own selection report), an unlabelled `AXRow` with text descendants, and a window at the desktop-icon level read through `tree_scope: desktop_surface`. The `notes_list()` seeds are neutral and the expected string is built from them. (harness case not yet run on a desktop — pending idle window) | Yes — the fixtures and the three cases are generic; the desktop-surface scenario is the only upstream-novel capability (it proves a non-Finder desktop row) |
| `fix(macos): render a menu item's fn modifier, and pin the chord constants on the fixture menu` (3d88d61d6) | fn modifier and menu depth pinned by the fixture menu bar. `MENU_MODIFIER_FN = 1 << 4` and `KEY_EQUIVALENT_MAX_DEPTH = 4` rested on two Notes items (`View > Enter Full Screen`, `Window > Move & Resize > Return to Previous Size`) and on unit tests that assert the constants against themselves. The harness fixture now declares an fn-modified equivalent at depth 4 and a second at depth 5: a chord at the not-key window is dispatched as the depth-4 item and is **not** dispatched as the depth-5 one, so both the bit and the bound are pinned generically. The renderer also learned the bit — an fn item listed as a bare letter named a chord that does not exist. (harness case not yet run on a desktop — pending idle window) | Yes — the fixture items, the harness assertions and the `menu_shortcut` fn glyph are all generic macOS truths |
| `fix(macos): say which signals the delivery probe compared, and which it could not` (bcd183fc5) | Every no-reaction reply in the 20260921-full leg named a fixed signal list: native-act-login-item/omp-1:24 claimed a "focused element" was watched on a chord that passed no element (633 ms = the blind budget, which is only chosen when no element state was readable), and no reply ever named the accessory-surface signal F2 added. Two spellings for one set across four tools (13 replies "element state", 8 "focused element"). The list is now derived from the probe's own record and uses the contract's signal tokens; SETTLE_BUDGET is unchanged — the loop already exits on the first watched change, and the leg paid it on 21 of 193 probed dispatches (41.8 s, 10.9 %). `click`'s no-op reason and pointerdown note now state a candidate instead of a cause (the bundle family says which toolkit drew a control, never what it listens for), and the phrase OMP's session matches on (`watched for <n> ms after the dispatch`) is pinned from the driver side. | yes — the sentence is a general honesty fix; BLIND_SETTLE_BUDGET's doc comment rides with it |
| `feat(macos): publish the window fact the menu-command reply prints` (b41946d94) | Give the chord→menu-command route a live seam, and publish the window fact the reply prints. The route was reverse-engineered from Notes (cmd+option+f, 0/12 at a not-key window vs 6/6 key) and shipped unit-covered only: harness_appkit_test.rs:2068-2070 records that the fixture has no second key-able window, so the substitution arm had never run end to end. The fixture's Window ▸ Arrange ▸ Left already validates on window.isKeyWindow; the second top-level window wave 10 added makes the whole route — disabled lookup, make-key, press, restore — a generic AppKit case, and its case now asserts the fields beside the sentence. Separately, OMP was recovering "was the app fronted" by regexing the driver's English because the closed projection drops key_window; it is now a published field (`ActionResult.key_window`, optional, `made_key` / `app_fronted` / `restored`). Regenerating `contract/manifest.json` also picked up `menu_opened` in the evidence signal enum, which wave 10 added without regenerating. (harness case not yet run on a desktop — pending idle window) | yes — the fixture case is upstream-shaped; the key_window field is a contract addition and should go as its own PR |
