...
//! The retained lease belongs to the authenticated session lifecycle,
//! including teardown and retry.
...
impl ManagedLease for WindowLease {
    fn stop(&mut self) -> Result<(), String> {
        ...
        if let Some(stream) = self.stream.as_mut() {
            ...
            let drained = Arc::get_mut(stream).is_some_and(|s| {
                s.remove_output_handler(self.output_id, SCStreamOutputType::Screen)
            });
            if !drained {
