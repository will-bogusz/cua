...
impl Tool for GetWindowStateTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let elements_json: Vec<serde_json::Value> = match (snapshot_id, tree_result.as_ref()) {
            (Some(sid), Some(r)) => {
                build_elements_array_with_policy(&r.nodes, Some(sid), |pointer| {
...
mod tests {
    ...
    fn structured_file_item_reports_confirm_and_open_actions() {
        let mut file = node(Some(0), "AXTextField", None, 0, None, None, vec![]);
        file.value = Some("Alpine source.txt".into());
        file.actions = vec!["AXOpen".into(), "AXConfirm".into()];
        let entries = build_elements_array_with_token(&[file], Some(1));
    ...
    /// Calendar puts an event's calendar membership in `AXHelp`, and Reminders
    /// puts "mark completed" there: a caller reading only the structured rows
    /// could not tell what a control does.
    #[test]
    ...
    fn background_file_actions_preserve_raw_capabilities_but_exclude_refused_open() {
        let mut file = node(Some(0), "AXTextField", None, 0, None, None, vec![]);
        file.actions = vec!["AXOpen".into(), "AXConfirm".into()];
        let rows = build_elements_array_with_policy(&[file.clone()], Some(1), |_| true);
        assert_eq!(
            rows[0]["actions"],
            serde_json::json!(["AXOpen", "AXConfirm"])
        );
    ...
    /// `elements_complete` is a promise about absence, so it needs a resolved
    /// window AND a walk that gave nothing up.
    #[test]
    fn completeness_is_promised_only_for_a_resolved_window_and_a_whole_walk() {
        assert!(elements_are_complete(true, Some(false)));
        // The walk hit a cap, ran out of budget, or could not read a child
        // list, so controls it never saw could still exist.
    ...
    #[test]
    fn an_expired_walk_never_promises_a_complete_element_set() {
        ...
        assert!(walk.truncated, "an expired walk enumerated nothing");
        assert!(!elements_are_complete(true, Some(walk.truncated)));
    }
}
