...
impl ExactWindowObservation {
    ...
    fn exact_window_front_in_process(self, window_id: u32) -> bool {
        self.target_visible_ordinary
            && self.process_frontmost_ordinary_window_id == Some(window_id)
    }
...
mod tests {
    ...
    fn another_applications_window_on_top_does_not_unverify_the_reveal() {
        ...
        assert_eq!(structured["exact_window_effect"]["frontmost_ordinary"], false);
        assert_eq!(
    ...
    fn a_sibling_window_of_the_same_application_in_front_is_only_partial() {
        assert_eq!(
            classify_exact_outcome(true, 42, 7, contested_observation(Some(7), Some(8), Some(8))),
            ExactOutcome::Partial
        );
    }
