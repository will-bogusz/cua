...
impl Drop for RuntimeSession {
    fn drop(&mut self) {
        ...
        // Take the admission guard here, not on the cleanup thread: a drain
        // that starts after shutdown has already claimed the write side would
        // otherwise run after shutdown returned, keeping the runtime alive
        // past its owner. If the guard is unavailable, shutdown is in progress
        // and ends this runtime's sessions itself.
        let Ok(admission) = self.runtime.lifecycle.clone().try_read_owned() else {
            return;
        };
        ...
        if let Err(error) = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // Release the runtime before admission so shutdown observes a
                // fully released runtime once the drain has completed.
                drop(runtime);
                drop(admission);
            })
        {
...
pub(crate) struct DriverRuntime {
    ...
    shutdown: AtomicBool,
    /// Set once explicit `shutdown` has drained native resources, so the
    /// finalizer does not schedule a second, unobservable cleanup.
    drained: AtomicBool,
...
impl Drop for DriverRuntime {
    fn drop(&mut self) {
        ...
        if self.drained.load(Ordering::Acquire) {
            // Explicit shutdown already drained everything this thread would.
            return;
        }
        let runtime_scope = self.compatibility_context.runtime_scope_key();
        let registry = self.registry.clone();
