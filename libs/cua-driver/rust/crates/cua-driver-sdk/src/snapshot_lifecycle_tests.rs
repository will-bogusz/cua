...
impl Tool for CaptureTool {
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        ...
        .await
        .unwrap();
        // The runtime owns the native work to completion even after its caller
        // vanished; the tool then sees that cancel before it would mint a token
        // nobody can hold, exactly as get_window_state does. A shutdown
        // interrupt is not that: the call still answers and publishes.
...
async fn sdk_cancelled_capture_does_not_publish_after_shutdown() {
    ...
    wait_for_closed_admission(&driver).await;
    // The vanished caller only requested cancellation: the runtime still owns
    // the admitted native work, so shutdown cannot return before it ends.
